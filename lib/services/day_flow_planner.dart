...
abstract final class DayFlowPlanner {
  ...
  static DayFlow plan({
    ...
    ItineraryPickContext pickCtx() => ItineraryPickContext(

          profile: p,

          pool: pool,

          tripUsed: excludePlaceIds,
          ...
          rotationSeed: dayIndex * 41 + pickCounter++,

          intentText: intentText.isNotEmpty ? intentText : p.tripFeel,
    ...
    LuxPlace? pick(
      ...
      final place = ItineraryPlacePicker.pickExperience(
      ...
      if (place != null) {
        ...
        cursor = LatLng(place.latitude, place.longitude);
    ...
    LuxPlace? pickDining() {
      ...
      final place = ItineraryPlacePicker.pickDining(ctx: pickCtx());
    ...
    if (TripPreferenceGates.allowsWellnessMorning(p)) {
    ...
    final middayCategories = _experienceCategoriesFor(
      p,
      isMiami: isMiami,
      includeFamily: TripPreferenceGates.wantsFamilyAttractions(p),
    );
    ...
    if (blocks.length < 2) {
      add(
        ...
        pick(
          middayCategories,
          tags: const ['iconic', 'trending', 'culture'],
        ),
    ...
    while (blocks.length < minStops) {
      ...
      final phase = fillPhases.firstWhere(
        ...
        (phase) => !blocks.any((b) => b.phase == phase),

        orElse: () => DayPhase.afternoon,

      );

      final filler = phase == DayPhase.evening

          ? pickDining()

          : ItineraryPlacePicker.pickDayFiller(ctx: pickCtx());

      if (filler == null) break;

      if (!used.contains(filler.id)) {

        used.add(filler.id);

        cursor = LatLng(filler.latitude, filler.longitude);
  ...
  static List<LuxPlaceCategory> _experienceCategoriesFor(
    TripProfile profile, {
    required bool isMiami,
    required bool includeFamily,
  }) {
    if (isMiami) {
      return [
        LuxPlaceCategory.adventure,
        LuxPlaceCategory.beach,
        LuxPlaceCategory.nature,
        if (includeFamily) LuxPlaceCategory.family,
        if (TripPreferenceGates.isFriendsOrAdultSocialTrip(profile))
